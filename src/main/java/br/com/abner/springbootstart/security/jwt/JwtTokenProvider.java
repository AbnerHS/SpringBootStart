package br.com.abner.springbootstart.security.jwt;
...
import br.com.abner.springbootstart.exceptions.InvalidJwtAuthenticationException;
...
public class JwtTokenProvider {
    ...
    private long validtyInMiliseconds = 3600000; // 1 hora
    ...
    private String getRefreshToken(String username, List<String> roles, Date now) {
        Date validity = new Date(now.getTime() + (validtyInMiliseconds * 3)); // 3 horas
        return JWT.create()
                .withClaim("roles", roles)
                .withIssuedAt(now)
                .withExpiresAt(validity)
    ...
    private DecodedJWT decodedToken(String token) {
        Algorithm algorithmDecode = Algorithm.HMAC256(secretKey.getBytes());
